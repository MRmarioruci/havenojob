...
class AppModel:
	def promptChatGPT(self, messages, temperature):
		try:
			...
			log('An error occured on chatcompletion')
			return None


	def getJobMatch(self, candidate_job_title:str, candidate_profile:str, job_title:str, job_description:str):
		...
		evaluation = self.promptChatGPT([
			...
			{"role": "system", "content": "Format your response like the following example: {'percentageLow': Number%, 'percentageHigh': Number%}."},
	...
	def generateProfileSummary(self, candidate_job_title:str, candidate_profile:str):
		...
		response['data'] = self.promptChatGPT([
			...
			{"role": "user", "content": f'candidate_job_title: {candidate_job_title}'},
			{"role": "user", "content": f'candidate_cv: {candidate_profile}'},
	...
	def generateJobDescriptionSummary(self, job_title:str, job_description:str):
		...
		response['data'] = self.promptChatGPT([
			...
			{"role": "system", "content": "The format should be something like; Job Title: Software Engineer \n Years of experience required: 6 etc."},
			{"role": "user", "content": f'job_title: {job_title}'},
